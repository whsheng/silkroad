...
V2 已在当前仓库内完成重构，并作为正式线上版本运行。

当前实现特点：

- 公开站使用 App Router
- 内容源迁移到 `content/`
- 多语言采用 `zh-CN` 与 `en` 并列路由
- SEO、sitemap、robots、hreflang 已接入
- 投稿页已接入 `submit -> GitHub Issue` 最小闭环
...
- CDN / 安全层：Cloudflare
- 代码仓库：GitHub
- 内容存储：Git 仓库中的 `content/`
- 投稿入口：站内表单
- 投稿收件箱：GitHub Issues
- 已发布内容：构建时静态输出

当前公开站前台不展示工程实现细节，相关说明统一保留在项目文档中。
...
当前线上投稿闭环采用“无数据库、低成本、可上线”的实现方式。

### 8.1 当前实现路径

用户提交：

- 用户在 `/zh-CN/submit` 或 `/en/submit` 填写站点信息

服务端处理：

- 表单提交到站内 API `/api/submissions`
- 服务端做基础字段校验
- 服务端做蜜罐反垃圾校验
- 服务端根据域名做基础去重
...
- 审核通过 GitHub Issues 承接
- 每次提交创建一条结构化 Issue
- 编辑后续基于 Issue 补充分类、简介、市场与平台信息

上线方式：

- 审核确认后，将内容写入 `content/`
- 合并到 `main` 后由 Vercel 自动重新部署
